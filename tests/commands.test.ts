...
describe('commands', () => {
  ...
  test('Get64 ignores State64 for tiles outside the requested range', () => {
    const seen: number[] = [];
    const cmd = Commands.Get64({ tileIndex: 2, tileCount: 2, width: 8, onResponse: (response) => {
      seen.push(response.tileIndex);
    } });
    const state64 = (tileIndex: number) => {
      const bytes = new Uint8Array(5 + 64 * 8);
      bytes[0] = tileIndex;
    ...
    decode(state64(2), { current: 0 }, continuation, Type.State64);
    decode(state64(0), { current: 0 }, continuation, Type.State64);
    assert.equal(continuation.expectMore, true);
    decode(state64(4), { current: 0 }, continuation, Type.State64);
    assert.equal(continuation.expectMore, true);
    const result = decode(state64(3), { current: 0 }, continuation, Type.State64);
    assert.equal(continuation.expectMore, false);
    assert.deepEqual(result.map((tile) => tile.tileIndex), [2, 3]);
