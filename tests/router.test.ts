...
describe('router', () => {
  ...
  test('batchSend calls onBatchSend when available', () => {
    const sentMessages: any[] = [];
    
    const router = Router({
      onSend() {
        assert.fail('onSend should not be called when onBatchSend is available');
      },
    ...
    assert.deepEqual(sentMessages[0].message, new Uint8Array([1, 2, 3]));
    ...
    assert.deepEqual(sentMessages[1].message, new Uint8Array([4, 5, 6]));
  ...
  test('batchSend falls back to individual onSend calls when onBatchSend not available', () => {
    const sentCalls: any[] = [];
    ...
    assert.deepEqual(sentCalls[0].message, new Uint8Array([1, 2, 3]));
    ...
    assert.deepEqual(sentCalls[1].message, new Uint8Array([4, 5, 6]));
    assert.equal(sentCalls[1].port, 56701);
    assert.equal(sentCalls[1].address, '192.168.1.101');
    assert.equal(sentCalls[1].serialNumber, undefined);
  });

  test('batchSend handles empty message array', () => {
    let onBatchSendCalled = false;
    ...
    assert.ok(onBatchSendCalled);
    assert.ok(!onSendCalled);
  });

  test('batchSend fallback handles empty message array', () => {
    let onSendCalled = false;
