 ...
 * same router) still works; and nothing here is required, since any runtime can
...
export async function openLan(options: OpenLanOptions = {}): Promise<LanInstance> {
  ...
  // event (dgram throws when the event has no listener), so forward or discard.
  socket.on('error', (err) => {
