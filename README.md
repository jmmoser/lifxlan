...
- `onError` — forwarded to the router: observes inbound datagrams that fail to decode and errors thrown by the `onMessage` tap. When omitted they're discarded, so a malformed packet or a buggy callback can neither crash the process nor stop reception.
