...
The optional `lifxlan/discovery` subpath packages the repeat-broadcast loop from the Quick Start. `discover()` broadcasts `GetService` immediately and then on a widening backoff, yielding devices as your handler registers them: known devices first, then new arrivals. The backoff starts at `intervalMs` (default 1s), quadruples per broadcast, and settles at `maxIntervalMs` (default 1 minute), so with the defaults broadcasts go out immediately, then after 1s, 4s, 16s, and every minute from there. The burst defeats UDP loss when it matters most; the capped heartbeat keeps a long-lived stream from broadcasting to every device on the network once a second. Set `maxIntervalMs` equal to `intervalMs` for a fixed interval. End to end:
...
LIFX's guidance is to send **at most 20 messages per second to any single device**. The library deliberately does not throttle, because every operation maps to exactly one packet and pacing policy (drop, queue, coalesce) belongs to your application, so staying under the limit is your code's job:
...
- **A packet was lost.** UDP is lossy even on a healthy network, so wrap sends in a retry loop with backoff (see [Error Handling with Retries](#error-handling-with-retries)).
